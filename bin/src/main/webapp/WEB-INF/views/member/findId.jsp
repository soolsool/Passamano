			...
			<div class="login-section my-6 pt-5 pb-5">
				...
				<form class="form-inline w-50 mx-auto" method="post">
					<div class="row mb-3">
						...
						<div class="col-sm-6">
							<input type="text" class="form-control input-sm" id="name" name="name" aria-label="Username" aria-describedby="basic-addon1">
						</div>
					</div>
					...
					<!-- 인증번호 버튼 클릭시 노출 -->
					<div class="row mb-3 verifyCode">
						<label for="phone" class="col-sm-3 col-form-label text-end verifyCode">인증번호</label>
						<div class="col-sm-6 verifyCode">
							<input type="text" class="form-control input-my-1" name="code" id="code" placeholder="인증번호" required="required">
							<div id="alertCode">
							</div>
						</div>
						<div class="col-sm-3 verifyCode">
					...
					<div class="row mb-3">
						...
						<div class="col-sm-6">
							<button type="button" class="btn btn-success my-2 w-100" id="btnGetId" onclick="getId()" disabled="disabled">아이디 확인</button>
						</div>
						<div class="col-sm-3">
						</div>
					</div>
...
<script type="text/javascript">
	...
	function getId(){
		...
		$.ajax({
			...
			data: JSON.stringify({'name': name, 'phone':phone}),
			success: function(id){
				$("#alertId").append("<p class='fs-6 fw-light'>회원님의 ID는 " + id+ "입니다.</p>");
			}
		});
	}
